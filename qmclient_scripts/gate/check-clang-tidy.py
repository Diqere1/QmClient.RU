...
COMPILER_MODE = os.environ.get("CLANG_TIDY_COMPILER", "clang-cl" if os.name == "nt" else "clang")
...
def require_llvm_major(command, display_name):
	result = subprocess.run([command, "--version"], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False)
	output = result.stdout
	if not re.search(rf"(version|LLVM version)\s+{re.escape(CLANG_VERSION)}\.", output):
		raise SystemExit(f"{display_name} must be LLVM/Clang {CLANG_VERSION}.x, got:\n{output}")
...
def main():
	...
	run(configure_command, cwd=BUILD_DIR)

	build_command = [cmake_bin, "--build", ".", "--config", "Debug", "--target", BUILD_TARGET, "--", "-k", "0"]
	return run(build_command, cwd=BUILD_DIR, log_path=BUILD_DIR / "clang-tidy.log")
