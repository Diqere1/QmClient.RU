...
`qmclient_scripts/gate/check_gate.py` 是 QmClient 的仓库级总入口。
...
- `checks/strict_build` 负责真实构建、MSVC `/analyze`、clang-tidy、ASan 与严格结果汇总
- 它是 `check_gate.py` 在 `default` / `full` 模式下自动调度的检查模块

总原则：

- 需要跑"仓库级门禁模式"时，用 `qmclient_scripts/gate/check_gate.py`
- `default` / `full` 模式会自动触发 `checks/strict_build`
...
`checks/strict_build` 在 `check_gate.py` 调度下执行严格构建。`check_gate.py` 提供：

- `--explain-scope`：打印纳入文件、排除文件和稳定 `reason`
- `--report-json-path`：输出统一 JSON 报告（含 `BaseRef`、`Summary`、`ScopedFiles`、`Items` 等）
...
- `qmclient_scripts/gate/check_gate.py` 负责仓库级模式编排，`checks/strict_build` 作为其调度模块维护严格检查语义
- `checks/strict_build` 已内联原严格构建逻辑，是统一的事实源
...
`check_gate.py` 的汇总和 JSON 报告现在应把失败至少分成三类：
