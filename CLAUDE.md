...
- Windows 上默认用 `qmclient_scripts\cmake-windows.cmd` 作为构建入口；常规构建/测试目录是 `cmake-build-release`，不要继续使用旧的 `build-ninja` 口径；只有已确认当前 shell 已注入可用的 VS/MSVC 环境时，才直接使用裸 `cmake`。
