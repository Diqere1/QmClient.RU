...
constexpr size_t MAX_BACKGROUND_VIDEO_QUEUED_FRAMES = 2;
...
void ResolveBackgroundEntitiesStoragePath(IStorage *pStorage, const char *pBackgroundEntities, const char *pDefaultExtension, char *pOut, int OutSize)
{
	...
	else if(str_startswith_nocase(pBackgroundEntities, "mapres/"))
	{
		str_format(pOut, OutSize, "%s%s", pBackgroundEntities, str_endswith_nocase(pBackgroundEntities, pExtension) ? "" : pExtension);
	}
	else
	{
...
bool CBackground::DecodeNextMediaFoundationVideoFrame(double TargetFrameTime, std::vector<uint8_t> &vFrameBuffer)
{
	...
	for(int Attempts = 0; Attempts < 16; ++Attempts)
	{
		...
		if(FAILED(Result))
		{
			log_warn("background/video-mf", "Could not read video sample: error=0x%08x text='%s' attempts=%d flags=0x%08x stream=%lu",
				(unsigned)Result,
				windows_format_system_message(Result).c_str(),
				Attempts + 1,
				StreamFlags,
		...
		if(FAILED(Result))
		{
			log_warn("background/video-mf", "Could not get sample buffer: error=0x%08x text='%s' attempts=%d flags=0x%08x timestamp=%lld",
				(unsigned)Result,
				windows_format_system_message(Result).c_str(),
				Attempts + 1,
				StreamFlags,
		...
		Result = pBuffer->QueryInterface(IID_PPV_ARGS(&p2DBuffer));
		if(SUCCEEDED(Result))
		{
			BYTE *pSourceData = nullptr;
			LONG SourceStride = 0;
			Result = p2DBuffer->Lock2D(&pSourceData, &SourceStride);
			if(SUCCEEDED(Result))
			{
