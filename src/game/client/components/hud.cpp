...
void CHud::RenderTextInfo()
{
	...
	if(RenderFps || RenderPred || RenderLoss)
	{
		...
		{
			FpsRectX = m_Width - 10.0f - DisplayFpsWidth;
			FpsRectY = 5.0f;
		}

		float PredRectX = 0.0f;
		float PredRectY = 0.0f;
		if(UseV2TextInfoLayout)
		{
		...
		{
			PredRectX = m_Width - 10.0f - DisplayPredWidth;
			PredRectY = RenderFps ? 18.0f : 5.0f;
		}

		float LossRectX = 0.0f;
		float LossRectY = 0.0f;
		if(UseV2TextInfoLayout)
		{
		...
		{
			LossRectX = m_Width - 10.0f - DisplayLossWidth;
			LossRectY = RenderPred ? PredRectY + 13.0f : (RenderFps ? 18.0f : 5.0f);
		}
	...
	if(RenderFps)
	{
		...
		{
			FpsX = m_Width - 10 - DisplayFpsWidth;
			FpsY = 5.0f;
		}
	...
	if(RenderPred)
	{
		...
		{
			PredX = m_Width - 10 - DisplayPredWidth;
			PredY = RenderFps ? 18.0f : 5.0f;
		}
		if(UseV2TextInfoLayout)
		{
	...
	if(RenderLoss)
	{
		...
		{
			LossX = m_Width - 10.0f - DisplayLossWidth;
			LossY = RenderPred ? 31.0f : (RenderFps ? 18.0f : 5.0f);
		}
...
void CHud::RenderMediaIsland()
{
	...
	const auto HudEditorScope = GameClient()->m_HudEditor.BeginTransform(EHudEditorElement::MediaIsland, EditorTransformRect);
	GameClient()->m_HudEditor.UpdateVisibleRect(EHudEditorElement::MediaIsland, EditorVisibleRect);
