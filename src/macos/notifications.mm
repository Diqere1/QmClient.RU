...
void NotificationsNotifyMacOsInternal(const char *pTitle, const char *pMessage)
{
	@autoreleasepool
	{
		NSString *pNsTitle = [NSString stringWithCString:pTitle ? pTitle : "" encoding:NSUTF8StringEncoding];
		NSString *pNsMsg = [NSString stringWithCString:pMessage ? pMessage : "" encoding:NSUTF8StringEncoding];
		...
		{
			NSLog(@"DDNet notification suppressed after macOS exception: %@ - %@", [pException name], [pException reason]);
		}
