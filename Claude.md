...
- RmlUI 相关的工作请优先参考 `docs\superpowers\API` 中的 API 文档，如缺少，请使用 context7 补充
...
- `RelWithDebInfo` 下若出现 `_ITERATOR_DEBUG_LEVEL` / `RuntimeLibrary` 不匹配，先检查是否是 C++ 与 Rust CRT 混链，不要直接判断源码损坏。
- RmlUI 相关的 Release 崩溃、黑屏、资源缺失或”Debug 正常但 Release 异常”问题，优先对 `build-ninja/` 执行 `--clean-first` 全量重建后再复现；旧的 `rmlui.lib`、资源同步或增量产物可能污染判断。
...
RmlUI / Release-only 问题需要先排除旧产物污染时：

```powershell
qmclient_scripts\cmake-windows.cmd --build build-ninja --clean-first --target game-client -j 10 | Select-Object -Last 30
```

Release 行为需要源码栈时，使用临时 Release+PDB 诊断目录：

```powershell
qmclient_scripts\cmake-windows.cmd -S . -B build-release-pdb -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXE_LINKER_FLAGS_RELEASE="/DEBUG /INCREMENTAL:NO"
qmclient_scripts\cmake-windows.cmd --build build-release-pdb --target game-client -j 10 | Select-Object -Last 30
```
...
- 运行命令（相对路径）：`.\build-ninja\DDNet.exe`
...
- `src/game/client/RmlUi/`：QmClient 新 UI 框架（实验中）
- `src/test/`：C++ 测试
