...
- 完整功能或改进完成后，如影响发布版本，按 MMP 规则更新版本号
- 实现用户要求并得到用户确认后，提交说明按 FEAT / FIX / DEL 分类概括

## 工作边界

- 优先遵循 DDNet 现有实现模式，不为了”现代化”重写既有代码
- 通用现代 C++ 最佳实践（见 context7）与 DDNet 既有风格或兼容性约束冲突时，优先服从 DDNet 约束
- 用户需求模糊时必须继续追问，直到需求、边界和验收条件清楚后再实现
- 新功能、玩法变化或较大行为改动，默认先讨论，不直接扩展实现    
- 默认不要修改根目录 `CMakeLists.txt`、协议字段、序列化布局或文件格式定义，除非任务明确要求
- QmClient 的配置项统一使用 `qm_` / `Qm` 前缀，不使用 `cl_` 前缀
- QmClient 自有功能在代码、测试、fixture、helper、文件名中默认使用 `Qm` 命名，不把 `QmClient` 写进新增标识符；只有项目名、产品名、既有公共类型或对外文本确实需要时才使用 `QmClient`
- 测试名要描述用户可见行为或核心规则，但不要把开发方法写进标识符；新增测试文件、测试套件、测试用例名中不要使用 `BDD`、`TDD`、`Scenario_` 这类词
...
- 修改 CRLF 文件时，新增行也必须是 CRLF，禁止在同一文件内混入 LF；如果编辑后出现 mixed line endings，必须在提交前归一化回原文件的换行格式
- 禁止使用 git checkout，如需要使用，使用 git restore，而且必须指定文件名，只允许回退需要改的文件
- git 提交的提交信息要按照 type(scope): content 格式来，并且需要符合中文提交，例如：fix(core): 修复核心逻辑
- 根据用户的系统来规范使用斜杠和反斜杠，不要未经思考就混用，比如 Windows 需要：使用反斜杠和驱动器字母在文件路径中
...
实现逻辑或给出补丁时，必须根据改动风险建议或执行验证，尤其覆盖：
...
- 构建并运行 `run_tests`
- 进行 client/server 实际联调
- 在可行时使用 ASan / UBSan / Valgrind 思路检查问题
- 检查格式化与风格脚本是否通过
...
列出问题后，还要给出总体评价：正确 / 需要修复 / 不安全，并补充简要说明和可选的重构建议。

审查行为规则：

- 关注真实问题，不要纠结纯外观层面的吹毛求疵
- 解释为什么这是问题，尤其是兼容性、预测一致性、热路径性能和生命周期问题
- 尽量给最小补丁建议
- 如果问题涉及 DDNet 历史约束，要明确指出这是项目约束，不是通用 C++ 教条
- 不要建议会大面积破坏现有风格的一刀切重构
- 如果不确定某个行为是否是历史兼容要求，要明确标注不确定性，不要武断下结论
...
- 新增或调整 `config_variables*.h`、生成配置、全局配置结构、跨大量翻译单元使用的头文件后，如果 Release 启动早期出现只在 `build-ninja` 复现、Debug 或新建 Release+PDB 目录不复现的崩溃，优先怀疑旧增量产物没有完整重编导致对象布局/内联代码不一致。先执行 `qmclient_scripts\cmake-windows.cmd --build build-ninja --target game-client --clean-first -j 10` 后再复现，不要直接把裸地址崩溃归因到图形驱动或最近运行时逻辑。
- 在 PowerShell/JSON/脚本字符串里写 Windows 路径时注意反斜杠转义，尤其 `\tclient` 里的 `\t` 可能被当成制表符导致读取不存在的路径。面向工具调用和脚本参数时优先使用正斜杠路径，例如 `src/game/client/components/tclient/tclient.cpp`。
