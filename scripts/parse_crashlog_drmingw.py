...
CRASH_FILENAME_PATTERN = re.compile(
    r"(DDNet|DDNet-Server)(_([0-9\.\-]+))?_((win32|win64)(-steam)?)(_([a-zA-Z0-9]+))?_crash_log_([0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2})_([0-9]+)_([0-9A-Fa-f]*)"
)
...
class ParsedFilename:
    def __init__(
        self, executable, version, platform, architecture, timestamp, commit
    ) -> None:
...
def determine_image_base(executable: Path) -> int:
    ...
    if not match:
        raise RuntimeError(
            "Could not determine ImageBase from objdump output. Make sure you installed GNU objdump."
        )
...
def determine_error_message(lines: list[str]) -> str:
    ...
    raise RuntimeError(
        "Crash log does not contain the error message of the crash.\nIt was likely not possible to determine a more detailed stack trace due to the severity of the crash."
    )


def download_symbols_executable(parsed_filename: ParsedFilename | None) -> Path:
    print(
        "Executable containing debug symbols was not specified. Trying to find it automatically..."
    )

    if not parsed_filename:
        raise RuntimeError(
            "Executable containing debug symbols cannot be identified, because the crash log filename has an unexpected format.\nMake sure to keep the original filename of the crash log, or specify the executable manually with the `--executable` parameter."
        )

    if parsed_filename.executable not in ["DDNet", "DDNet-Server"]:
        raise RuntimeError(
            "Executable containing debug symbols cannot be identified. Only official releases of DDNet and DDNet-Server have downloadable debug symbols.\nSpecify the executable manually with the `--executable` parameter if you have it."
        )
    ...
    if not parsed_filename.version:
        ...
        raise RuntimeError(
            "Executable containing debug symbols cannot be identified, because the crash log filename has an unsupported format: missing version string.\nMake sure to keep the original filename of the crash log, or specify the executable manually with the `--executable` parameter."
        )
    if not parsed_filename.architecture:
        print(
            "Warning: The crash log filename does not specify the architecture, so we will assume it is not ARM64."
        )
        print(
            "         Try manually using the ARM64 executable if you see unresolved symbols below."
        )
    ...
    if not symbols_folder_path.is_dir():
        ...
        symbols_url = urllib.parse.urljoin(
            "https://ddnet.org/downloads/symbols/", symbols_archive_name
        )
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to download debug symbols. They are likely not available for this crash dump.\nCheck your internet connection and make sure the crash log has its original filename."
            ) from error
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to extract debug symbols. The debug symbols archive may be corrupted.\nPlease report this error if it persists."
            ) from error
    ...
    if not symbols_executable.is_file():
        raise RuntimeError(
            f"Folder for matching symbols '{symbols_folder_path}' was found in cache but it does not contain the executable '{executable_name}'."
        )
    print(
        f"Found matching symbols in cache: {symbols_executable.relative_to(symbols_executable.parent.parent)}"
    )
...
def process_crash_log(crash_log: Path, executable: Path | None):
    ...
    if parsed_filename:
        ...
        print(
            "  Warning: Crash log filename has unexpected format. Make sure to keep its original name."
        )
    ...
    for line in lines:
        ...
        if executable:
            ...
            if (
                executable_release_match
                and executable_release_match.group(1) == executable.name
            ):
    ...
    if found_unresolved_symbols:
        print()
        print(
            "Warning: Found unresolved symbols. Make sure to select the matching executable."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Symbolize a crash log produced by Dr. Mingw ExcHndl."
    )
    parser.add_argument("crash_log", type=Path, help="Path to crash log file (.RTP)")
    parser.add_argument(
        "--executable", type=Path, help="Path to executable with debug symbols (.exe)"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Print full stack trace on errors"
    )
