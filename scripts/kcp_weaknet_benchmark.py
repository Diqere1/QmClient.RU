...
def process_cpu_seconds(process):
	...
	if os.name == "nt":
		...
		if not kernel32.GetProcessTimes(int(process._handle), ctypes.byref(creation), ctypes.byref(exit_time), ctypes.byref(kernel), ctypes.byref(user)): # pylint: disable=protected-access
			return 0.0
...
def launch_server(args, tmpdir, case_name):
	...
	cmd = [
		...
		config.format(loss=args.loss, jitter=args.jitter, rtt=args.rtt, reorder=args.reorder)
