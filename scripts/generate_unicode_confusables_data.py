...
def generate_decompositions():
    ...
    def apply(chars, replacements):
        return [d for c in chars for d in replacements.get(c, [c])]
...
""")
    ...
    for k in sorted(decompositions):
        ...
        length_idx = len_set.index(len(d))
        print(f"\t{{{decomposition_offsets[i]}, {length_idx}}},")
    print("};")
    print()
