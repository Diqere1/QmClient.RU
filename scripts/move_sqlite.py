...
def transfer(file_from, file_to, date, keep_timestamp_utc):
    ...
    for table in TABLES:
        cursor_to.execute(
            f'INSERT INTO {table} SELECT * FROM {table}_backup WHERE Timestamp < DATETIME("{date}", "utc")'
        )
    cursor_to.close()
    conn_to.commit()
    ...
    for table in TABLES:
        ...
        if sqlite_table_exists(cursor_from, backup_table):
            cursor_from.execute(
                f'DELETE FROM {backup_table} WHERE Timestamp < DATETIME("{date}", "utc")'
            )
    cursor_from.close()
    conn_from.commit()
    conn_from.close()
...
def main():
    ...
    date = (datetime.now() - timedelta(minutes=args.backup_timeout)).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    print(
        f"{num} new entries in backup database found ({num['record_race']} ranks, {num['record_teamrace']} teamranks, {num['record_saves']} saves)"
    )
    print(
        f"Moving entries from {os.path.abspath(args.f)} to {os.path.abspath(args.to)}"
    )
    print(
        "You can use the following commands to import the entries to MySQL (using https://github.com/techouse/sqlite3-to-mysql/):"
    )
    print()
    print(
        f"sqlite3mysql --sqlite-file {os.path.abspath(args.to)} --ignore-duplicate-keys --mysql-insert-method IGNORE --sqlite-tables record_race record_teamrace record_saves --mysql-password 'PW2' --mysql-host 'host' --mysql-database teeworlds --mysql-user teeworlds"
    )
    print(
        f"When the ranks are transferred successfully to mysql, {os.path.abspath(args.to)} can be removed"
    )
    print()
