        ...
        if line[0] == "[":
            ...
            if not allow_context:
                raise LanguageDecodeError(
                    "Context not allowed in this file", fileobj.name, index
                )
...
def language_index():
    with open("data/languages/index.txt", encoding="utf-8") as f:
        return {
            key: value for (key, _), value in decode(f, 3, allow_context=False).items()
        }
...
def languages():
    ...
    langs = {
        "data/languages/" + key + ".txt": [key] + elements
